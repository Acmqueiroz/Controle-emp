...
export const buscarDadosSemana = async (dataInicio, dataFim) => {
    ...
    const q = query(empadasCollection, where('data', '>=', dataInicio.toISOString().split('T')[0]), where('data', '<=', dataFim.toISOString().split('T')[0]));
    try {
        const querySnapshot = await getDocs(q);
        // Agrega por sabor somando freezer/estufa/perdas através dos documentos
        const agregadoPorSabor = new Map();
...
export const buscarSaldoInicialPorSabor = async (dataReferencia) => {
    const empadasCollection = collection(db, 'contagem_diaria');
    const dataStr = dataReferencia.toISOString().split('T')[0];
    const q = query(empadasCollection, where('data', '<', dataStr), orderBy('data', 'desc'), limit(7) // última semana aprox
    );
    const saldos = {};
...
export const buscarSaldoFinalPorSabor = async (dataInicio, dataFim) => {
    const empadasCollection = collection(db, 'contagem_diaria');
    const q = query(empadasCollection, where('data', '>=', dataInicio.toISOString().split('T')[0]), where('data', '<=', dataFim.toISOString().split('T')[0]), orderBy('data', 'desc'));
    const saldos = {};
    const snap = await getDocs(q);
    snap.forEach((docSnap) => {
        const dados = docSnap.data();
...
export const calcularTotais = (dados) => {
    let totalFreezer = 0;
    let totalEstufa = 0;
    let totalPerdas = 0;
    dados.forEach((item) => {
...
export const buscarDadosDoMes = async (ano, mes) => {
    const empadasCollection = collection(db, 'contagem_diaria');
    const inicio = new Date(ano, mes - 1, 1).toISOString().split('T')[0];
    const fim = new Date(ano, mes, 0).toISOString().split('T')[0];
    const q = query(empadasCollection, where('data', '>=', inicio), where('data', '<=', fim), orderBy('data', 'asc'));
    const snap = await getDocs(q);
    const contagens = [];
    const dias = [];
    snap.forEach((d) => {
...
export const calcularResumoMensal = (contagens) => {
...
export const buscarDocumentoAnterior = async (dataStr) => {
    const empadasCollection = collection(db, 'contagem_diaria');
